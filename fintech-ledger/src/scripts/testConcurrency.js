...
const run = async () => {
  ...
  console.log(
    `   Tokens are ${senderToken === receiverToken ? 'SAME ' : 'DIFFERENT '}`
  );
  ...
  if (senderAccountId === receiverAccountId) {
    ...
    console.error(
      'Check if both users are properly created as separate users.'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('Some requests failed (this is okay if expected):', e.message);
  }
